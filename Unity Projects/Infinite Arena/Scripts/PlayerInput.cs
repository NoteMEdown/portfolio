...
{
    public class PlayerInput : vp_Component
    {
        ...
        [Tooltip("Sensitivity of the Rotaion")]
        [SerializeField]
        private int x_sensitivity = 10;
        [Tooltip("Sensitivity of the Camera")]
        [SerializeField]
        private int y_sensitivity = 10;
        [Tooltip("Gun to be rotated")]
        [SerializeField]
        private GameObject gunObject;
        [Tooltip("Speed at which the fun rotates")]
        [SerializeField]
        private float gunRotationSpeed = 20f;
        [Tooltip("Holds Player information")]
        [SerializeField]
        private PlayerManager player;
        [Tooltip("Prefab for the basic shot")]
        [SerializeField]
        private GameObject basicShotPrefab;
        [Tooltip("Prefab for the missile shot")]
        [SerializeField]
        private GameObject missileShotPrefab;
        [Tooltip("How far in front of the gun the shot spawns")]
        [SerializeField]
        private const float shotOffset = 1f;
        ...
        protected override void Update()
        {
            ...
            if (showCursor) // if we are showing cursor
            {
                Cursor.lockState = CursorLockMode.None; // Freely move mouse off screen
                Cursor.visible = true; // Make cursor visible
                m_MoveVector = Vector2.zero; // stop moving
                return; // Process no further inputs
            }
            else // not showing cursor
            {
                Cursor.lockState = CursorLockMode.Confined; // cursor confined to window
        ...
        protected override void FixedUpdate()
        {
            ...
            if(!showCursor && photonView.IsMine)
                InputCamera();
        }
        ...
        {
            /// <summary>
            /// -1 to 1, -1 = mouse on full left side of screen, 1 = mouse on full right side of screen
            /// </summary>
            float x_scale = Mathf.Clamp(((Input.mousePosition.x / Screen.width) - 0.5f) * 2, -1f, 1f); // results in -1 to 1 based on where in the screen you are
            /// <summary>
            /// -1 to 1, -1 = mouse on full bottom of screen, 1 = mouse on full top of screen
            /// </summary>
            float y_scale = Mathf.Clamp(((Input.mousePosition.y / Screen.height) - 0.5f) * -2, -1f, 1f);
            //           Debug.Log(Input.mousePosition);
            //           Debug.Log("x_scale: " + x_scale + "\ny_scale: " + y_scale);
            // Start camera speghetti here!
            if(Mathf.Abs(y_scale) > 0.1f)
            {
                playerCamera.transform.localRotation = Quaternion.Euler(playerCamera.transform.localRotation.eulerAngles.x + (y_scale * y_sensitivity * Time.deltaTime), 0f, 0f);
            }
            if (Mathf.Abs(x_scale) > 0.1f)
            {
                transform.rotation = Quaternion.Euler(0f, transform.rotation.eulerAngles.y + (x_scale * x_sensitivity * Time.deltaTime), 0f);
            }

            Vector3 screenPos = new Vector3(Input.mousePosition.x, Input.mousePosition.y, 10f);
            screenPos = playerCamera.ScreenToWorldPoint(screenPos);
            gunObject.transform.LookAt(screenPos);
        }

        private void ManageCamera()
        {
            if (Input.GetKeyDown(KeyCode.M)) // If this button is pressed
            {
                showCursor = !showCursor; // Toggle if we are showing cursor
                GameManager.Instance.leaveButton.SetActive(!GameManager.Instance.leaveButton.activeSelf);
            }
        }
        ...
        protected virtual void InputAttack()
        {
            ...
            if (Input.GetButtonDown("Fire1"))
            {
                ...
                {
                    player.ShotFired();
                    GetComponent<PhotonView>().RPC("ShootBasicShot", RpcTarget.All, Array.IndexOf(PhotonNetwork.PlayerList, PhotonNetwork.LocalPlayer) + 1);
                    //  GameObject shot = PhotonNetwork.Instantiate(basicShotPrefab.name, gunObject.transform.position + (gunObject.transform.forward * shotOffset), gunObject.transform.rotation);
                    //  shot.GetComponent<BasicShot>().SetPlayer(player);
                    return; // return so you cant shoot and rocket at the same time
                }
            }
            ...
            {
                // Missile Logic here
                if (player.TryMissileShot())
                {
                    GetComponent<PhotonView>().RPC("ShootMissileShot", RpcTarget.All, Array.IndexOf(PhotonNetwork.PlayerList, PhotonNetwork.LocalPlayer) + 1);
                }
            }
        }

        [PunRPC]
        void ShootBasicShot(int playerNum, PhotonMessageInfo info)
        {
            GameObject shot = Instantiate(basicShotPrefab, gunObject.transform.position + (gunObject.transform.forward * shotOffset), gunObject.transform.rotation);

            shot.GetComponent<BasicShot>().SetPlayer(player);
            shot.GetComponent<BasicShot>().SetPlayerNum(playerNum);
            float timePassed = (float)(PhotonNetwork.Time - info.timestamp);
            shot.transform.position = shot.transform.position + shot.GetComponent<Rigidbody>().velocity * timePassed;
        }

        [PunRPC]
        void ShootMissileShot(int playerNum, PhotonMessageInfo info)
        {
            GameObject shot = Instantiate(missileShotPrefab, gunObject.transform.position + (gunObject.transform.forward * shotOffset), gunObject.transform.rotation);

            shot.GetComponent<MissileShot>().SetPlayerNum(playerNum);
            float timePassed = (float)(PhotonNetwork.Time - info.timestamp);
            shot.transform.position = shot.transform.position + shot.GetComponent<Rigidbody>().velocity * timePassed;
        }
...
//        }
        ...
        public void SetGameRunning(bool _gameRunning)
        {
